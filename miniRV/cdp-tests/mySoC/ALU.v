...
module ALU (
    input wire rst, input wire clk, input wire [4:0] op,
    input wire [31:0] a, input wire [31:0] b,
    output reg [31:0] c, output reg br, output wire busy
);
    ...
    always @(*) begin
        c=0;
        case(active_op)
            `ALU_ADD: c=a+b; `ALU_SUB: c=a-b; `ALU_AND: c=a&b; `ALU_OR: c=a|b; `ALU_XOR: c=a^b;
            `ALU_SLL: c=a<<b[4:0]; `ALU_SRL: c=a>>b[4:0]; `ALU_SRA: c=$signed(a)>>>b[4:0];
            `ALU_SLT: c={31'h0,$signed(a)<$signed(b)}; `ALU_SLTU: c={31'h0,a<b};
            `ALU_MUL: c=mul_res[31:0]; `ALU_MULH: c=mul_res[63:32]; `ALU_MULHU: c=mulu_res[63:32];
    ...
    always @(*) begin
        br=0;
        case(op)
            `ALU_EQ: br=a==b; `ALU_NE: br=a!=b;
            `ALU_LT: br=$signed(a)<$signed(b); `ALU_LTU: br=a<b;
            `ALU_GE: br=$signed(a)>=$signed(b); `ALU_GEU: br=a>=b;
        endcase
    end
    always @(posedge clk or posedge rst) begin
        if(rst) begin op_r<=`ALU_ADD; div_q_neg<=0; div_r_neg<=0; div_by_zero<=0; dividend_r<=0; end
        else begin
            if(mul_start|mulu_start|div_start|divu_start) op_r<=op;
            else if(!busy) op_r<=`ALU_ADD;
            if(div_start) begin div_q_neg<=a[31]^b[31]; div_r_neg<=a[31]; div_by_zero<=b==0; dividend_r<=a; end
        end
    end
